...
## Behavior notes
  ...
  - There is **no `do merge` command** — `gg do merge` was folded into `gg do publish --merge-only` (below), so there is exactly one flow. Its implementation lives on as the plain tool `MergeFlow` (`tools/merge_flow.dart`, formerly the `DoMerge` command): no `Command`/`DirCommand` base, no `argParser`, no CLI surface — every parameter arrives from `DoPublish`, which is its only caller. It drops the `.gg/ticket.json` / `.gg/.ticket.json` ticket marker before merging, so it never lands on the main branch. `do_publish` calls the same removal (`MergeFlow.removeTicketJson`) right at the start, before the version bump — the marker must neither ride into the release commits the merge puts on main nor ship inside the package the registry upload publishes afterwards. gg_multi no longer writes that marker into a repo at all (a ticket's `ticket.json` stays in the ticket folder and is never committed), so this removal now only scrubs branches that an older gg pushed.
  - **`MergeFlow` never checks a branch out** — HEAD stays on the feature branch in both flows, so editor tooling (e.g. the Dart extension of VS Code) never sees the old main state in the worktree and cannot rewrite lock files mid-release. The **local flow** builds the squash commit with git plumbing on the feature branch: `git rev-parse HEAD:` (the feature tree — which IS the merge result, because gg_merge's `CanMerge` refuses a branch that is behind main; a belt-and-braces `merge-base --is-ancestor` re-checks against the local main ref) → `git commit-tree <tree> -p <main> -m <message>` → `git update-ref refs/heads/<main> <squash> <oldTip>` (the old tip as expected value, so a concurrent ref move fails loudly). gg_merge's `DoMerge`/`LocalMerge` (which check main out) are used for the **pull-request path only**. The **local main ref sync** (`git branch -f`, no checkout) replaces the former checkout+pull: fast-forward when local main is an ancestor of origin/main, force-move with a warning when the divergence is gg bookkeeping / lock-file drift only, hard error naming the files when real local commits would be lost, ref creation when local main is missing, and a no-op when origin has no main (the local flow pushes it later). The pull-request path runs it after the provider merged (with a fresh fetch — the provider merged after the last one); `DoPublish`'s tag step is the only checkout of a release, and it happens once main already carries the release.
